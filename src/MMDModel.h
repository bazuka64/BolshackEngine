...
struct MMDModel {
	...
	struct Morph {
		std::wstring name;
		char type; // 0:ÉOÉãÅ[Év, 1:í∏ì_, 2:É{Å[Éì, 3:UV, 4:í«â¡UV1, 5:í«â¡UV2, 6:í«â¡UV3, 7:í«â¡UV4, 8:çﬁéø
		int offset_count;
...
#pragma pack(1)
	struct RigidBody {
		...
		float restitution;
		float friction;
		char type; // 0:É{Å[Éìí«è](static) 1:ï®óùââéZ(dynamic) 2:ï®óùââéZ + Boneà íuçáÇÌÇπ
		btRigidBody* btBody;
		glm::mat4 fromBone;
		glm::mat4 fromBody;
	};
#pragma pack()
	...
	void Update(float dt) {
		...
		for (Bone& bone : bones) {
			...
			// CCD-IKÇÃäpìxêßå¿ÇÃÇΩÇﬂÇ…ë´ÇëOÇ…90ìxâÒì]Ç≥ÇπÇÈ
			if (bone.name == L"âEë´" || bone.name == L"ç∂ë´")
